...
export const CanvasAIGame: React.FC<CanvasAIGameProps> = ({ onBackToSetup }) => {
  ...
  useEffect(() => {
    ...
    let lastAIMove: { row: number; col: number; player: number } | null = null;
    let winningPositions: { row: number; col: number }[] = [];
    let jumpOffset = 0;
    let jumpDirection = 1;
    const jumpHeight = Math.min(15, cellSize * 0.2); // Scale jump height for AI disc
    const jumpSpeed = 0.4; // Slow jumping for AI disc
    let pulseScale = 1; // Scale factor for winning discs' pulsing animation
    let pulseDirection = 1;
    const pulseSpeed = 0.02; // Speed of pulsing animation
    const pulseMaxScale = 1.2; // Max scale for pulsing
    const pulseMinScale = 0.8; // Min scale for pulsing
    ...
    function drawBoard() {
      ...
      // Update AI disc jump animation
      if (gameActive && lastAIMove && lastAIMove.player === 2 && currentPlayer === 1) {
        jumpOffset += jumpDirection * jumpSpeed;
        if (jumpOffset >= jumpHeight) jumpDirection = -1;
        if (jumpOffset <= -jumpHeight) jumpDirection = 1;
      } else {
        jumpOffset = 0; // Reset jump when game is inactive, no AI move, or AI's turn
        jumpDirection = 1;
      }

      // Update winning discs pulse animation
      if (!gameActive && winningPositions.length > 0) {
        pulseScale += pulseDirection * pulseSpeed;
        if (pulseScale >= pulseMaxScale) pulseDirection = -1;
        if (pulseScale <= pulseMinScale) pulseDirection = 1;
      } else {
        pulseScale = 1; // Reset pulse when no winning positions
        pulseDirection = 1;
      }
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          // Apply jumping animation only to the most recent AI disc
          if (gameActive && lastAIMove && lastAIMove.row === row && lastAIMove.col === col && lastAIMove.player === 2 && currentPlayer === 1) {
            offsetY += jumpOffset;
          }

          // Apply pulsing animation to winning positions
          const isWinning = winningPositions.some(pos => pos.row === row && pos.col === col);
          if (isWinning && !gameActive) {
            scale = pulseScale;
          }

          const centerX = col * cellSize + cellSize / 2 + offsetX;
          const centerY = row * cellSize + cellSize / 2 + offsetY;
          const radius = (cellSize / 2 - Math.min(4, cellSize * 0.05)) * scale;
    ...
    async function playAIMove() {
      ...
      if (bestMove !== null && isValidMove(bestMove)) {
        ...
        if (result.success) {
          lastAIMove = { row: result.row, col: bestMove, player: 2 }; // Set lastAIMove to the newly dropped disc
    ...
    function handleHumanMove(event: MouseEvent | TouchEvent) {
      ...
      if (isValidMove(col)) {
        ...
        if (result.success) {
          lastAIMove = null; // Reset AI move to stop jumping
          drawBoard();
